...
c	note: I the amperstand, &, its only place is the 6th column 
	...
	real*8 c0(4000),c1(4000),c2(4000),c3(4000)
...
c	x=(sra*cd*ci+sd*si)/cd*cra
c	write(*,*) x	
          if (ra(i).lt.90.0) then
		mu=-acos(x)*180.0/pi+node(i) + 360.0
         elseif (ra(i).gt.275.0) then
              mu=acos(-x)*180.0/pi+node(i)+180.0
         else if (node(i).gt.180.0) then
	      mu=-acos(x)*180.0/pi+node(i)
	 else	
              mu=acos(x)*180.0/pi+node(i)
         endif
	...
	do 20 m=1,5
	 ...
	 hockey=abs(mu-a(l))
c	 write(*,*) mu
	if (hockey.gt.300.0) mu=mu-360.0
c		write(*,*) mu
